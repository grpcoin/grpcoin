...
func NewPubSub(src <-chan Quote, stop func()) *PubSub {
	p := &PubSub{src: src, stop: stop,
		subs: make(map[chan<- Quote]bool)}
	go p.fanout()
	return p
}
...
func (p *PubSub) Sub(ch chan<- Quote) {
	p.mu.Lock()
	p.subs[ch] = true
	p.mu.Unlock()
}
...
func (p *PubSub) Unsub(ch chan<- Quote) {
	p.mu.Lock()
	defer p.mu.Unlock()
	_, ok := p.subs[ch]
	if !ok {
